...
const CustomButtonIcon = (props: CustomButtonIconProps) => {
 ...
 return (
  <TouchableOpacity className={`p-2 rounded-full bg-slate-100 flex-row justify-center items-center ${className}`} onPress={handlerPress}>
   {text && type === 'eliminated' && <Text className=" mr-2 text-red-900 font-semibold"> {text} </Text>}
   {text && type === 'refresh' && <Text className=" mr-2 text-gray-600 font-semibold"> {text} </Text>}
...
const Icon = (props: IconProps) => {
 ...
 if (type === satusList.arrowLeft) return <ArrowLeftCircleIcon size={50} color={'rgb(156, 163, 175)'} />;
 if (type === satusList.refresh) return <ArrowPathIcon size={25} color={'rgb(156, 163, 175)'} />;
 if (type === satusList.expand) return <ArrowsPointingOutIcon size={25} color={'rgb(156, 163, 175)'} />;
